Vue.component('app-mini-cart', {
	...
	methods: {
		...
		addProduct(product) {
			...
			if (find) {
				this.$parent.getJson(`/minicart-add/${product.id_product}`)
					.then(data => {
						if (data.result === 1) {
							find.quantity++;
						}
		...
		remProduct(product) {
			let find = this.cartList.find(el => el.id_product === product.id_product);
			if (find.quantity === 1) {
				this.$parent.getJson(`/minicart-del/${product.id_product}`)
					.then(data => {
						if (data.result === 1) {
	...
	template: `
		<div class="cart">
			...
			<div class="drop__cart" v-show="cartShow">
				...
				<a href="/checkout" class="cart__button">Checkout</a>
				<a href="/cart" class="cart__button">Go to cart</a>
			</div>					
		</div>`
});
...
Vue.component('app-main-cart', {
	...
	<div class="medium__button conteiner">
		<a href="/index" class="cart__button-big">cLEAR SHOPPING CART</a>
		<a href="/catalog" class="cart__button-big">cONTINUE sHOPPING</a>
	</div>
	<!-- средние кнопки -->

	<!--форма заказа-->
	<form action="/cart" class="result conteiner" method="POST">
		<div class="result__item">
			...
			<input type="text" name="name" class="result-input" placeholder="You Name">
			<input type="tel" name="phone" class="result-input" placeholder="You Phone">
		...
		<div>
			<div class="result__item-grey">
				...
				<button type="submit" class="cart__button-proceed" >MAKE ORDER</button>
			</div>			
		</div>
		<input type="hidden" name="order" value="exist">
	...
	template: `
		<div class="cart__pos">
			<div class="cart__pos-left">
				...
				<div>
					<a :href="'/single/' + product.product_id" class="cart__pos-title">{{product.product_name}}</a>
